...
class SyncTriggersGrvPostingTest extends TestCase
{
    ...
    public function test_a_synced_po_receipt_with_a_rejected_quantity_records_the_discrepancy(): void
    {
        $tenantId = 'tenant-e2e-grv-reject';
        $token = $this->actingDeviceToken($tenantId);

        $supplier = Supplier::create(['id' => (string) Str::uuid(), 'business_id' => $tenantId, 'name' => 'Acme Supplies']);
        $po = PurchaseOrder::create([
            'id' => (string) Str::uuid(), 'business_id' => $tenantId, 'supplier_id' => $supplier->id,
            'supplier_name' => $supplier->name, 'po_number' => 'PO-0002', 'status' => 'sent',
            'total_ordered' => 100, 'total_received' => 0, 'created_by_user_id' => (string) Str::uuid(),
        ]);
        $product = Product::create([
            'id' => (string) Str::uuid(), 'business_id' => $tenantId, 'name' => 'Widget',
            ...
            ->postJson('/api/v1/sync/push', [
                'records' => [[
                    ...
                    'payload' => [
                        ...
                        'user_id' => (string) Str::uuid(),
                        'quantity_rejected' => 2,
                        'rejection_reason' => 'Wrong item delivered',
                    ],
                    'updated_at' => now()->toIso8601String(),
                ]],
            ]);
        ...
        $grv = GoodsReceivedVoucher::where('purchase_order_id', $po->id)->firstOrFail();
        $item = GrvItem::where('grv_id', $grv->id)->firstOrFail();
        $this->assertSame('22.0000', $item->quantity_received);
        $this->assertSame('20.0000', $item->quantity_accepted);
        $this->assertSame('2.0000', $item->quantity_rejected);
        $this->assertSame('Wrong item delivered', $item->rejection_reason);
    }
